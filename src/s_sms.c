	...
	const void *event_info, void *user_data)
{
	...
	struct tnoti_sms_umts_msg incoming_msg;
	int sca_length = 0;

	dbg("Enter");

	memset(&incoming_msg, 0x0, sizeof(struct tnoti_sms_umts_msg));
	...
	tcore_server_send_notification(tcore_plugin_ref_server(tcore_object_ref_plugin(co)),
		...
		sizeof(struct tnoti_sms_umts_msg), &incoming_msg);
	...
	int data_len, const void *data, void *user_data)
{
	...
	struct tresp_sms_send_umts_msg send_sms_resp;
	dbg("Enter");
	...
	if (ur) {
		tcore_user_request_send_response(ur,
			...
			sizeof(struct tresp_network_search), &send_sms_resp);
	} else {
		err("ur is NULL");
	}
}
...
{
	const struct treq_sms_send_umts_msg *send_sms_req = NULL;
	char *at_cmd = NULL;
